...
MongoDB.prototype.updateAll = function (model, query, update, callback) {
	if(query && query.where){
		query = query.where;
	}
	
    this.collection(model).update(query, update, {multi:true, upsert:false, safe:true}, function (err, m) {
        callback(err);
    });
};
