...
AbstractClass.updateAll = function updateAll(query, update, callback) {
    if (stillConnecting(this.schema, this, arguments)) return;

    var Model = this;
    if (this.schema.adapter.updateAll) {
		this.schema.adapter.updateAll(Model.modelName, query, update, false, true, callback);
	}else{
		callback("updateAll not found on adapter");
	}
}
